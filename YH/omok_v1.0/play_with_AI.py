...
class MyApp(QWidget):
    ...
    def mousePressEvent(self, e):
        ...
        if self.game_end == 0:
            ...
            """best_po = [[i, np.argmax(result_mat[i])] for i in range(len(result_mat))]
            best_po_prob = [np.round(result_mat[i][best_po[i][1]], 2) for i in range(len(result_mat))]"""
            best_po = [[i, np.argmax(result_mat[i])] for i in range(len(result_mat))]
            best_po_prob = [np.round(result_mat[i], 2) for i in range(len(result_mat))]


            print(best_po)
            [print(i) for i in best_po_prob]
